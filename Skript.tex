...
\subsubsection{Definition}
Sei \phi\in End_K(V) und \mu_\phi(x)^m_1...p_s(x)^{m_s} mit den Eigenfaktoren p_1(x),...,p_s(x) und mit m_i\geq 1.
...
(b) Gen(\phi,p_i(x))=BigKer(\phi-\lambda\cdot id_V)=Ker(\phi-\lambda\cdot id_V)^m=\underset{Hom(\phi, \lambda)}{Gen(\phi, \lambda)}. TODO PFEIL m>>0 BEI m
\subsubsection{Lemma}
Sei p(x)\in K[x]\{0} ein Vielfaches von \mu_\phi(x). Es gelte p(x)=q_1(x)q_2(x) mit ggT(q_1(x),q_2(x))=1. Ferner sei W_1=Ker(q_1(\phi)) und W_2=Ker(q_2(\phi)). Dann gilt:
(a) Für jedes f\in K[x] sind W_1 und W_2 f(\phi)-invariant.
(b) Für i \neq j ist q_i(\phi)|W_j: W_j \rightarrow W_j ein Isomorphismus. TODO Einschränkung
(c) V=W_1 o+ W_2
(d) Ist p(x)=\mu_\phi(x), so gilt q_i(x)=\mu_{\phi|W_i}(x) für i =1,2. TODO Einschränkung
\ul{Beweis:}
...
Hieraus folgt W_1 SCHNITT W_2 = Ker(q_1(\phi)) SCHNITT Ker(q_2(\phi))={0} nach (*) und dager ist q_i(\phi)|W_j:W_j\rightarrow W_j injektiv, also bijektiv. TODO Einschränkung
...
"d" Sei q_i'(x) das Minimalpolynom von \phi|W_i. Nach Def. von W_j gilt q__i'(x)|q_i(x), denn q_i(\phi|W_i)=q_i(\phi)|W_i=0. Also sind auch q_1'(x) teilerfremd und q_1'(x)q_2'(x)=\mu_\phi(x)=q_1(x)q_2(x), also q_1'(x)=q_1(x) und q_2'(x)=q_2(x). TODO Einschränkung
\qed
\subsubsection{Definition}
...
\subsubsection{Beispiel}
Sei \phi\in End_\mathbb{Q}(\mathbb{Q\^3) mit M_\xi^\xi(\phi)= TODO MATRIX. Dann golt M_\xi^\xi(\phi^2)=TODO MATRIX und \phi^3=0, also nix(\phi)=3.
\subsubsection{Lemma}
Seien U', U'' zwei \phi-invariante K-Untervektorräume von V mit V=U' +o U''.
Sei \phi'=\mu_{\phi |U'} und \phi''=\mu_{\phi |U''}. Dann gilt \mu_\phi(x)=kgV(\mu_{\phi |U'}(x),\mu_{\phi |U''}(x)).\\
\ul{Beweis:}\\
Es gilt \mu_\phi(\phi)=0. Also folgt \mu_\phi(\phi|U')=0 und \mu_\phi(\phi | U'')=0. Also ist \mu_\phi ein gemeinsames Vielfaches von \mu_{\phi'} und \mu_{\phi''}. Umgekehrt sei f(x)=kgV(\mu_{\phi'},\mu_{\phi''}(x)). Zu zeigen: f(\phi)=0. Betrachte f(\phi)\U'=f(\phi | U')=0 da f ein Vielfaches von \mu_{\phi'} ist. Genauso gilt f(\phi) | U''=f(\phi|U'')=0. Wegen V=U' o+ U'' folgt f(\phi)=0, also \mu_\phi |f.
\qed
\subsubsection{Theorem (Die verallgemeinerte Eigenraumzerlegung / Die Hauptraumzerlegung / Die Primärzerlegung von \(\phi\))}
Sei \phi\in End_K(V) und \mu_\phi(x)=p_1(x)^{m_1}...p_s(x)^{m_s} die Zerlegung in Eigenfaktoren.
(a) V=Gen(\phi,p_1(x)) o+ ... o+ Gen(\phi, p_s(x)).
(b) Die Einschränkung von p_i(\phi) auf \underbrace{Gen(\phi,p_i(x))}_{G_i} ist nilpotent mit nix(p_i(\phi)| G_i)=m_i.
...
\ul{s > 1}: Verwende das Lemma mit q_1(x)=p_1(x)^{m_1}...p_{s-1}(x)^{m_{s-1}} und q_2(x)=p_s(x)^{m_s}. Nach Teil (c) des Lemmas folgt V=Ker(q_1(\phi)) o+ Ker(q_2(\phi))\underset{I.V.}{=}(Ker(p_1(\phi))^{m_1} o+ ... o+ Ker(p_{s-1}(\phi)^{m_{s-1}}) o+ Ker(p_s(\phi)^{m_s}). Nach Teil (d) des Lemmas ist jeweils p_i(x)^{m_i} das Minimalpolynoom von p_i| G_i, Dies beweist (b). Teil (c) folgt aus Teil (b) des Lemmas. Teil (d) folgt aus (b).
\qed
TODO UNDERSET
\end{document}
...
TODO: \equals =^
c_ Teilemenge _c andersrum (c+, +c)
o+ innere Summe
~ über Buchstaben
SCHNITT zu umgredehtem u
Itemize
>> "für n groß genug" d.h. es gibt ein m_0\in \mathbb{N}, sodass dies für alle m\geq m_0 gilt.
